...
# 2) grab the generated SW → static/app/app-sw.js
COPY --from=builder /app/frontend/dist/app/app-sw.js \
     backend/server/static/app/app-sw.js

# 3) the Workbox runtime imported by app-sw.js (still lives at dist root)
COPY --from=builder /app/frontend/dist/workbox-*.js \
     backend/server/static/app/

# sanity check: make sure app-sw.js landed where Express expects it
RUN test -f backend/server/static/app/app-sw.js \
    || (echo "❌ app-sw.js missing!" && ls -lR backend/server/static/app && exit 1)
