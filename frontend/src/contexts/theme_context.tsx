...
export const ThemeProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  console.log(`%c[ThemeProvider Render] Rendering provider. Current theme state: ${theme}`, 'color: blue');


  // Effect to apply the class to <html> and update localStorage
  useEffect(() => {
    // *** ADD LOGS ***
    console.log(`%c[ThemeProvider useEffect] ---- START ---- (Theme: ${theme})`, 'color: green; font-weight: bold;');
    const root = document.documentElement; // Get <html> element
    console.log(`%c[ThemeProvider useEffect] <html> classes BEFORE change: "${root.className}"`, 'color: green;');
    ...
    if (theme === 'light' || theme === 'dark') {
      ...
      console.log(`%c[ThemeProvider useEffect] Applied class: "${theme}"`, 'color: green;');
    } else {
       console.warn(`[ThemeProvider useEffect] Invalid theme value: "${theme}", no class applied.`);
    }

    console.log(`%c[ThemeProvider useEffect] <html> classes AFTER change: "${root.className}"`, 'color: green;');
    ...
    console.log(`%c[ThemeProvider useEffect] ---- END ----`, 'color: green; font-weight: bold;');
    // *** END ADD LOGS ***
