...
class NBAFeatureGenerator:
    ...
    def __init__(self, debug=False):
        ...
        """
        self.debug = debug
        # Define league averages for use in fallback scenarios
    ...
    def compute_basic_features(self, df):
        ...
        # Calculate total scores if not present (vectorized operations)
    ...
    def compute_rolling_averages(self, df, window_size=5):
        ...
        try:
            ...
            # Vectorized rolling average calculation
            ...
            # Update only where we have valid averages
    ...
    def calculate_rest_features(self, df, max_lookback_days=30):
        ...
        try:
            ...
            # Find previous game dates for each team
            ...
            # Apply reasonable bounds and handle missing values
        ...
        try:
            ...
            for i in range(1, 4):
                ...
                if all(col in result_df.columns for col in [home_col, away_col, next_home_col, next_away_col]):
                    # Vectorized calculation of momentum between quarters
            ...
            # Calculate cumulative momentum - this section could be vectorized but needs careful handling
            weights = np.array([0.2, 0.3, 0.5])
            for idx, row in result_df.iterrows():
                current_quarter = int(row.get('current_quarter', 0))
                
                # Skip if not enough quarters played
                if current_quarter < 2:
                    continue
                    
                # Calculate weighted momentum based on available quarters
                if current_quarter == 2:
    ...
    def compute_matchup_features(self, df, max_lookback=5):
        ...
        try:
            ...
            # Create team pair identifier (e.g., "TeamA_TeamB") - always sorted to handle home/away reversals
            result_df['team_pair'] = result_df.apply(
                lambda row: '_'.join(sorted([row['home_team'], row['away_team']])),
                axis=1
            )
            
            # Ensure dates are properly formatted and sort
            result_df['game_date'] = pd.to_datetime(result_df['game_date'], errors='coerce')
            result_df = result_df.sort_values('game_date')
            ...
            for team_pair in team_pairs:
                ...
                # Calculate matchup differential consistently (always from perspective of first home team)
                ...
                # Calculate average differential from previous games
            ...
            # Map differences back to original dataframe
    ...
    def compute_quarter_specific_features(self, df):
        ...
        result_df = df.copy()
        try:
            # Calculate differentials at different stages of the game
    ...
    def add_advanced_features(self, df):
        ...
        try:
            ...
            # Determine momentum indicator based on available data
            if 'cumulative_momentum' in result_df.columns:
                ...
                # Use the most recent momentum available based on current quarter
            ...
            # Calculate interaction between score and momentum
    ...
    def validate_features(self, df):
        ...
        # Recalculate key features if missing
        ...
        # Ensure all numeric features are properly typed and bounded
        numeric_features = [
        ...
        # Bound quarter scores to reasonable values
        for q in range(1, 5):
...
class EnsembleWeightVisualizer:
    ...
    def __init__(self, error_history=None):
        """Initialize with optional error history."""
    ...
    def _standard_weights(self, quarter, **kwargs):
        """Standard weighting strategy from original model."""
        if quarter == 1:
            return 0.80, 0.20
        elif quarter == 2:
            return 0.85, 0.15
    ...
    def _conservative_weights(self, quarter, **kwargs):
        """Conservative strategy that relies more heavily on main model."""
        if quarter == 1:
            return 0.90, 0.10
        elif quarter == 2:
            return 0.92, 0.08
    ...
    def _aggressive_weights(self, quarter, **kwargs):
        """Aggressive strategy that gives more weight to quarter-specific models."""
        if quarter == 1:
            return 0.70, 0.30
        elif quarter == 2:
            return 0.75, 0.25
    ...
    def _adaptive_weights(self, quarter, **kwargs):
        """Adaptive strategy based on historical error patterns."""
        ...
        quarter_weight = main_error / total_error if total_error > 0 else 0.2
        
        # Apply bounds to ensure weights stay reasonable
    ...
    def compare_strategies(self, current_quarter, score_differential=0, momentum=0, 
        ...
        """
        Compare different weighting strategies for a given game context.
        
        Args:
            current_quarter: Current quarter (1-4)
            score_differential: Point difference (home - away)
            momentum: Momentum value (-1 to 1)
            main_uncertainty: Uncertainty of main model prediction
            quarter_uncertainty: Uncertainty of quarter model prediction
            
        Returns:
            DataFrame with strategies and their weights
        """
        results = []
        kwargs = {
    ...
    def visualize_weight_progression(self, strategy='standard'):
        """
        Visualize how weights change as a game progresses.
        
        Args:
            strategy: Name of weighting strategy to visualize
    ...
    def visualize_all_strategies(self):
        """
        Visualize weight progression for all strategies.
    ...
    def track_weight_usage(self, game_id, quarter, main_weight, quarter_weight, prediction_error=None):
        """
        Track weights used and resulting error for later analysis.
        
        Args:
            game_id: Identifier for the game
            quarter: Current quarter
            main_weight: Weight applied to main model
            quarter_weight: Weight applied to quarter model
            prediction_error: Error after applying weights (if known)
        """
    ...
    def update_weights_from_feedback(self, recent_window=10):
        """
        Update error history based on recent prediction results.
        
        Args:
            recent_window: Number of recent games to consider
            
        Returns:
            Updated error history dictionary
        """
        ...
        # Update error history for main model
        for q, error in quarter_errors.items():
            if q in self.error_history['main_model']:
                # Blend new and historical errors with more weight on recent
    ...
    def analyze_weight_performance(self):
        """
        Analyze correlation between weights and prediction errors.
        ...
        for quarter, quarter_df in history_df.groupby('quarter'):
            # Calculate correlation between weights and errors
...
def dynamic_ensemble_predictions(main_prediction, quarter_prediction, current_quarter,
    ...
    5. Prediction uncertainties (if available)
    6. Different weighting strategies
    ...
    """
    # Initialize ensemble weight visualizer
    weight_viz = EnsembleWeightVisualizer(error_history)
    
    # Get initial weights based on selected strategy
    ...
    # Adjust confidence based on prediction discrepancy and momentum
...
class QuarterSpecificModelSystem:
    ...
    def __init__(self, feature_generator=None):
        ...
        # Add model cache for performance
    ...
    def predict_quarter(self, X, quarter):
        ...
        # Use league average if both models failed
        if prediction is None:
    ...
    def predict_remaining_quarters(self, game_data, current_quarter):
        ...
        for q in range(current_quarter + 1, 5):
            ...
            # Update features for next quarter's prediction
            X[f'home_q{q}'] = pred_score
            
            # Update derived features based on new quarter prediction
            if q == 2:
    ...
    def predict_final_score(self, game_data, main_model_prediction=None):
        ...
        # Calculate current home score from quarters played
        home_score = sum([float(game_data.get(f'home_q{q}', 0) or 0) for q in range(1, min(current_quarter + 1, 5))])
        
        # Use provided main model prediction or estimate based on current pace
        ...
        # For pre-game, just return the main model prediction
        if current_quarter <= 0:
        ...
        # Get ensemble prediction using dynamic weighting
        ...
        # Prepare detailed breakdown for explanation
        breakdown = {
            'main_model': main_model_prediction,
...
class PredictionUncertaintyEstimator:
    ...
    def __init__(self):
        ...
        # Track prediction intervals and actual outcomes
    ...
    def calculate_prediction_interval(self, prediction, current_quarter,
        ...
        Args:
            ...
            momentum: Momentum value (-1 to 1, optional).
        ...
        # Apply progressive narrowing based on quarter
        narrowing_factor = 1.0 - (current_quarter * 0.15)
        interval_half_width *= max(0.4, narrowing_factor)  # Don't go below 40%
    ...
    def update_error_metrics(self, errors_by_quarter):
        """
        Update internal error metrics based on new data.
        
        Args:
            errors_by_quarter: Dictionary mapping quarters to lists of errors
        """
        for quarter, errors in errors_by_quarter.items():
            if errors:
                self.historical_errors[quarter].extend(errors)
                # Only keep recent errors (last 100)
                self.historical_errors[quarter] = self.historical_errors[quarter][-100:]
                
                # Update MAE and STD for this quarter
    ...
    def record_interval_coverage(self, quarter, lower, upper, actual):
        """
        Record whether an actual value falls within the predicted interval.
        
        Args:
            quarter: Game quarter
            lower: Lower bound of prediction interval
            upper: Upper bound of prediction interval
            actual: Actual final score
        """
    ...
    def visualize_confidence_intervals(self, predictions_df):
        ...
        for i, (_, game) in enumerate(predictions_df.iterrows()):
            ...
            lower, upper, _ = self.calculate_prediction_interval(
                prediction, quarter, score_margin, momentum
            )
            ...
            ax.add_patch(Rectangle((i-0.2, lower), 0.4, upper-lower, alpha=0.2, 
                                 color=colors.get(quarter, '#000000')))
            
            # Add current score indicator if available
            current_score = game.get('home_score', None)
            if current_score is not None and current_score > 0:
                ax.scatter(i, current_score, marker='o', s=80, facecolors='none', 
                          edgecolors='green', linewidth=2, alpha=0.8)
    ...
    def create_confidence_indicator(self, prediction, lower_bound, upper_bound, 
        ...
        """
        Create a visual confidence indicator for use in a UI.
        ...
        Args:
            ...
            current_quarter: Current quarter of the game
            home_score: Current home score (if available)
            width: Width of indicator in pixels
            height: Height of indicator in pixels
            
        Returns:
            SVG string of the confidence indicator
        """
        interval_width = upper_bound - lower_bound
        
        # Normalize the prediction and interval to the width
        score_range = max(160, upper_bound + 10) - max(0, lower_bound - 10)
        ...
        # Calculate confidence percent (narrower interval = higher confidence)
        base_width = {0: 30, 1: 26, 2: 22, 3: 18, 4: 14}
        confidence = max(0, min(100, 100 - (interval_width / base_width.get(current_quarter, 25) * 100)))
        
        # SVG construction
        svg = f"""<svg width="{width}" height="{height}" viewBox="0 0 {width} {height}">
        <!-- Confidence interval -->
        <rect x="{lower_x}" y="{height/2 - 5}" width="{upper_x - lower_x}" height="10" 
              fill="{color}" fill-opacity="0.2" stroke="{color}" stroke-width="1" />
        ...
        if home_score is not None and home_score > 0:
            ...
            svg += f"""
            <!-- Current score -->
            <circle cx="{score_x}" cy="{height/2}" r="4" fill="none" stroke="green" stroke-width="2" />
            """
    ...
    def dynamically_adjust_interval(self, prediction, current_quarter, historic_accuracy):
        """
        Dynamically adjust confidence interval based on historic accuracy.
        
        Args:
            prediction: Predicted final score
            current_quarter: Current quarter of the game
            historic_accuracy: Dictionary mapping quarters to accuracy metrics
            
        Returns:
            tuple: (lower_bound, upper_bound, confidence)
        """
        # Use base interval calculation with score_margin=None
        lower, upper, width = self.calculate_prediction_interval(prediction, current_quarter)
        
        # If we have historic accuracy data, adjust the interval
        if historic_accuracy and current_quarter in historic_accuracy:
            accuracy = historic_accuracy[current_quarter]
            
            # If our intervals have been too wide (overconfident), widen them
            coverage = accuracy.get('coverage_pct', 95)
            if coverage < 90:
                widening_factor = (95 - coverage) / 50  # 0% to 10% widening
                width *= (1.0 + widening_factor)
                lower = prediction - width/2
                upper = prediction + width/2
            
            # If our intervals have been too narrow (underconfident), narrow them
            elif coverage > 98:
                narrowing_factor = (coverage - 95) / 300  # 0% to 1% narrowing
                width *= (1.0 - narrowing_factor)
                lower = prediction - width/2
                upper = prediction + width/2
        
        # Calculate confidence percentage (narrower interval = higher confidence)
...
def generate_enhanced_predictions(live_games_df, main_model):
    ...
    if missing_features:
        ...
        # Create a condensed feature set if needed
        available_features = [f for f in expected_features if f in features_df.columns]
        if len(available_features) >= 8:  # Minimum viable feature set
    ...
    # Calculate error history if validation data is available
    ...
    for i, (idx, game) in enumerate(features_df.iterrows()):
        ...
        # Get ensemble prediction and breakdown
        ...
        # Get dynamic interval adjustment if available
        ...
        # Generate confidence indicator for UI
        ...
        # Add quarter-specific predictions if available
...
def validate_enhanced_predictions(num_test_games=20):
    ...
    # Test predictions at different points in each game
    for _, game in test_games.iterrows():
        actual_home_score = game['home_score']
        
        # For each quarter (0=pre-game, 1-4=quarters)
        for test_quarter in range(0, 5):
            # Create simulated game state at this quarter
            sim_game = {
            ...
            # Fill in quarters that have been played
            for q in range(1, 5):
                if q <= test_quarter:
            ...
            # Get expected features for main model
            expected_features = feature_generator.get_expected_features(
                enhanced=hasattr(main_model, 'feature_importances_') and len(main_model.feature_importances_) > 8
            )
    ...
    # Calculate error metrics by quarter
...
def get_recommended_model_params(quarter, model_type=None):
    ...
    # Quarter-specific default recommendations
    if quarter == 1:
        return {
 ...
 # Early Quarter Model Enhancement 
class EarlyQuarterModelOptimizer:
    ...
    """
    def __init__(self, feature_generator=None):
        """Initialize with optional feature generator."""
        self.feature_generator = feature_generator or NBAFeatureGenerator(debug=False)
        self.models = {
            'q1': {},
            'q2': {}
        }
        # Define specialized feature sets for early quarters
    ...
    def initialize_xgboost_models(self):
        """Initialize XGBoost models for Q1 and Q2 with different configurations."""
        try:
            import xgboost as xgb
            
            # Q1 models with different configurations
    ...
    def train_models(self, training_data, target_col='home_score'):
        """
        Train all early quarter models with different feature sets.
        
        Args:
            training_data: DataFrame with features for training
            target_col: Target column name or column name prefix
        ...
        # Make sure feature generator has been used
        ...
        # Train Q1 models with different feature sets
        if self.models['q1']:
            ...
            if q1_target in training_data.columns:
                ...
                for feature_set_name, features in self.q1_feature_sets.items():
                    ...
                    # Train each model with this feature set
    ...
    def evaluate_models(self, test_data, target_col='home_score'):
        ...
        Args:
            test_data: DataFrame with features for testing
            target_col: Target column name or column name prefix
        ...
        # Evaluate Q2 models
        q2_target = f"{target_col[:-5]}q2" if target_col.endswith('score') else 'home_q2'
        if q2_target in test_data.columns:
            for model_key, model_info in self.models['q2'].items():
                for feature_set_name, features in self.q2_feature_sets.items():
                    model_id = f"{model_key}_{feature_set_name}"
                    if model_id in self.models['q2']:
                        model_data = self.models['q2'][model_id]
                        model = model_data['model']
                        features = model_data['features']
                        ...
                        if all(f in test_data.columns for f in features):
                            ...
                            print(f"Q2 {model_key} with {feature_set_name} features: RMSE={rmse:.2f}, MAE={mae:.2f}")
    ...
    def compare_to_baseline(self, eval_df, baseline_rmse):
        """
        Compare model performance to baseline (current model).
        
        Args:
            eval_df: DataFrame with evaluation results
            baseline_rmse: Dictionary with baseline RMSE by quarter
        ...
        comparison['baseline_rmse'] = comparison['quarter'].apply(
            lambda q: baseline_rmse.get(q[1], 7.5)  # Q1 -> 1, Q2 -> 2
        )
        ...
        # Sort by improvement
        comparison = comparison.sort_values(['quarter', 'pct_improvement'], ascending=[True, False])