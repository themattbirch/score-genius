...
# Import your upsert function – make sure it’s configured to upsert to "nba_live_player_stats"
    ...
    for game in live_games:
        ...
        if 'response' in player_stats:
            for stat in player_stats['response']:
                ...
                if 'minutes' in stat:
                    stat['minutes_numeric'] = convert_minutes(stat['minutes'])
                    # Optionally, remove the raw 'minutes' field
                    del stat['minutes']
                # Upsert the player stat record into the "nba_live_player_stats" table
                result = upsert_live_player_stats(game_id, stat)
                print(f"Upsert result for {stat.get('player', {}).get('name', 'Unknown')}: {result}")
        else:
