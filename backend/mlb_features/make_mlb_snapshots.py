...
def make_mlb_snapshot(
    ...
    logger.info(f"COLUMNS AVAILABLE in df_features BEFORE handedness: {df_features.columns.tolist()}")
    ...
    logger.info("Adding display-only handedness features for game %s...", game_id_str)

    # Step 6A: Determine which handedness columns we NEED for this game
    if is_historical_game:
        home_hand_col = "home_starter_pitcher_handedness"
        away_hand_col = "away_starter_pitcher_handedness"
    else:
        home_hand_col = "home_probable_pitcher_handedness"
        away_hand_col = "away_probable_pitcher_handedness"

    # Step 6B: THE DEFINITIVE FIX
    # We now know the feature engine may corrupt the *values* in these columns to NaN.
    # Unconditionally overwrite the columns in df_features with the known-good, original
    # values from df_game. This guarantees the correct data is passed to the transform.
    logger.info("Ensuring pitcher handedness values are correct before transform...")
    df_features[home_hand_col] = df_game[home_hand_col].values
    df_features[away_hand_col] = df_game[away_hand_col].values

    # Step 6C: Call the transform
    df_features_with_handedness = handedness_transform(
        df=df_features.copy(),
        mlb_pitcher_splits_df=df_pitcher_splits,
        home_team_col_param="home_team_norm", # <--- CORRECT
        away_team_col_param="away_team_norm", # <--- CORRECT
    ...
    logger.info(f"RPC for season {season_year} returned {len(df_rpc_adv_team)} rows.")
    if not df_rpc_adv_team.empty:
        # Crucially, let's see the keys we are supposed to be matching on
        logger.info(f"Unique 'team_norm' keys from RPC: {df_rpc_adv_team['team_norm'].unique()}")
    ...
    # --- ADD THIS DIAGNOSTIC LOGGING ---
    logger.info(f"Looking for home_team_norm: '{home_team_norm}'")
    logger.info(f"Looking for away_team_norm: '{away_team_norm}'")
    # --- END DIAGNOSTIC LOGGING ---
    ...
    # --- ADD THIS DIAGNOSTIC LOGGING ---
    if home_adv_rpc_data.empty:
        logger.warning(f"Could not find home team '{home_team_norm}' in RPC data. Using defaults.")
    if away_adv_rpc_data.empty:
        logger.warning(f"Could not find away team '{away_team_norm}' in RPC data. Using defaults.")
    # --- END DIAGNOSTIC LOGGING ---
    ...
    headlines = [
        ...
        {"label": "Home Avg Runs Scored Vs Starting Pitcher's Hand", "value": round(float(row.get("h_team_off_avg_runs_vs_opp_hand", MLB_DEFAULTS.get("mlb_avg_runs_vs_hand", 0.0))), 2)},
        {"label": "Away Avg Runs Scored Vs Starting Pitcher's Hand", "value": round(float(row.get("a_team_off_avg_runs_vs_opp_hand", MLB_DEFAULTS.get("mlb_avg_runs_vs_hand", 0.0))), 2)},
    ]
    # Rounding for headlines
    logger.debug("Applying standardized rounding to headline stats...")
    logger.info(f"🕵️ Raw Form Win% Diff before rounding: {headlines[1]['value']}")
    for h in headlines:
        if h['label'] == "Rest Advantage (Home)":
            h['value'] = int(h['value'])
        elif h['label'] == "Form Win% Diff":
            # Show three decimals instead of two
            h['value'] = round(float(h['value']), 3)
